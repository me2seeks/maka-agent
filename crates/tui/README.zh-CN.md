...
再次运行复用独立连接与历史；退出保留数据，本次启动的 Host 随 launcher 退出关闭。该目录包含凭据和聊天数据，不要分享或提交。已有 Host 若不兼容则拒绝，不自动升级。当前验证了真实 Host 启动及取消配置；真实服务商鉴权与生成需要你本机配置后验证。权限回答仍需后续实现，工具可能等待授权。
...
原生 `--connect` 仍是显式选定一个已有会话的低层入口；开发 launcher 提供启动阶段的选择和创建。聊天内不包含会话切换、附件、权限回答、
插件 UI 或自动恢复。权限/交互请求需在现有 Maka 客户端处理。
发送文本及正文单块上限 16 KiB，呈现历史最多 128 块／256 KiB；历史超额会中止连接并提示，不能视作完整历史。
Linux 隔离 Host + FakeBackend PTY 测试覆盖真实收发与停止；不代表真实模型、IME 或跨平台验收。
...
M0 仍需同版本 Companion echo、拥塞和生命周期测试、真实 Host 只读连接、复制／IME 与实际终端检查、跨平台验证和延迟／资源测量。后续阶段还包括结构化草稿、真实权限／停止／恢复、Markdown／搜索、完整命令与工作流、打包。目前没有新增依赖 notices 产物或发布二进制集成。不能据此切换默认 TUI 或宣称重写完成。
